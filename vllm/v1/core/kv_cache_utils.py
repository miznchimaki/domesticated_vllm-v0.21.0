...
def unify_kv_cache_spec_page_size(
    ...
    # # GRM_HYBRID_KV_PAGE_PADDING_PATCH_V1
    #
    # vLLM 0.21.0 normally enlarges block_size only when a
    # smaller page divides the largest page exactly.
    # Qwen3.5 hybrid Attention + Mamba/GDN on MLU can produce
    # non-integral page ratios. AttentionSpec and MambaSpec
    # already support page_size_padded, so prefer direct
    # physical-page padding while preserving token block sizes.
    if not kv_cache_spec:
    ...
    # Only legacy/unpaddable specs may require an LCM target.
    ...
    for layer_name, layer_spec in kv_cache_spec.items():
        ...
        # Preferred path for FullAttentionSpec / MambaSpec.
        if hasattr(layer_spec, "page_size_padded"):
            try:
                new_spec = replace(
                    layer_spec,
        ...
        # Preserve original vLLM block-size scaling for legacy specs.
        layer_page_size = layer_spec.page_size_bytes
        if target_page_size % layer_page_size != 0:
            raise NotImplementedError(
                "GRM hybrid KV page compatibility could not unify "
                f"layer={layer_name!r} page_size={layer_page_size} "
                f"to target_page_size={target_page_size}."
            )
        ...
        if new_spec.page_size_bytes != target_page_size:
            raise RuntimeError(
                "GRM hybrid KV page compatibility produced an "
                f"unexpected page size for {layer_name!r}: "
    ...
    final_page_sizes = {
        ...
        for spec in new_kv_cache_spec.values()
    }

    if final_page_sizes != {target_page_size}:
        raise RuntimeError(
            "GRM hybrid KV page compatibility failed final "
            f"validation: {sorted(final_page_sizes)}"
        )

    logger.info(
        "Unified hybrid KV-cache physical page sizes to %d bytes "
        "(original page sizes: %s).",
        target_page_size,
        sorted(page_sizes),
    )
