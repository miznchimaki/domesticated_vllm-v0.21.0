...
class RayDistributedExecutor(Executor):
    ...
    def _execute_dag(
        ...
        if not self.has_connector:
            ...
            # Drain non-output-rank workers' deques to prevent pinned
            # memory leak. In async-scheduling mode, only refs[0] is
            # consumed by the caller. Non-output-rank workers'
            # _execute_model_outputs deques are never drained, causing
            # AsyncModelRunnerOutput objects (which hold pinned host
            # memory from non-blocking D2H copies) to accumulate
            # indefinitely.
            if self.scheduler_config.async_scheduling:
                for ref_fn in refs[1:]:
                    ref_fn()
