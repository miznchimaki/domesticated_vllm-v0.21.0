...
# ---------------------------------------------------------------------------
# Tests: coerce_to_schema_type (replaces _convert_param_value)
# ---------------------------------------------------------------------------
...
class TestConvertParamValue:
    ...
    def test_null(self, parser):
        ...
        assert coerce_to_schema_type("null", "integer") is None
    ...
    def test_integer_invalid_falls_back_to_str(self, parser):
        assert coerce_to_schema_type("abc", "int") == "abc"

    def test_number_float(self, parser):
        assert coerce_to_schema_type("3.14", "number") == pytest.approx(3.14)
    ...
    def test_boolean_true(self, parser):
        ...
        assert coerce_to_schema_type("1", "bool") is True

    def test_boolean_false(self, parser):
        assert coerce_to_schema_type("false", "boolean") is False
        assert coerce_to_schema_type("False", "bool") is False

    def test_object_valid_json(self, parser):
        assert coerce_to_schema_type('{"k": 1}', "object") == {"k": 1}
    ...
    def test_array_valid_json(self, parser):
        assert coerce_to_schema_type("[1, 2]", "array") == [1, 2]

    def test_unknown_type_tries_json_then_string(self, parser):
        assert coerce_to_schema_type("123", "unknown") == 123
        assert coerce_to_schema_type("hello", "unknown") == "hello"
...
def test_convert_param_value_single_types(parser):
    """Test coerce_to_schema_type with single type parameters."""
    # Test string type
    assert coerce_to_schema_type("hello", "string") == "hello"
    assert coerce_to_schema_type("123", "string") == "123"

    # Test integer type - valid integers
    assert coerce_to_schema_type("123", "integer") == 123
    assert coerce_to_schema_type("456", "int") == 456
    # Invalid integer should return original string (due to exception catch)
    assert coerce_to_schema_type("abc", "integer") == "abc"

    # Test float/number type
    assert coerce_to_schema_type("123.45", "float") == 123.45
    assert (
        coerce_to_schema_type("123.0", "number") == 123
    )  # Should be int when whole number
    assert coerce_to_schema_type("123.5", "number") == 123.5
    # Invalid float should return original string
    assert coerce_to_schema_type("abc", "float") == "abc"
    ...
    assert coerce_to_schema_type("yes", "boolean") == "yes"
    assert coerce_to_schema_type("no", "bool") == "no"
    ...
    assert coerce_to_schema_type('{"key": "value"}', "object") == {"key": "value"}
    assert coerce_to_schema_type("[1, 2, 3]", "array") == [1, 2, 3]
    # Invalid JSON should return original string
    assert coerce_to_schema_type("{invalid}", "object") == "{invalid}"

    # Test fallback for unknown type (tries json.loads, then returns original)
    assert coerce_to_schema_type('{"key": "value"}', "unknown") == {
        "key": "value"
    }
    assert coerce_to_schema_type("plain text", "unknown") == "plain text"


def test_convert_param_value_multi_typed_values(parser):
    """Test coerce_to_schema_type with multi-typed values (list of types)."""
    # Test with list of types where first type succeeds
    assert coerce_to_schema_type("123", ["integer", "string"]) == 123
    assert coerce_to_schema_type("true", ["boolean", "string"]) is True
    assert coerce_to_schema_type('{"x": 1}', ["object", "string"]) == {"x": 1}

    # Test with list of types where first type fails but second succeeds
    # "abc" is not a valid integer, so should try string next
    assert coerce_to_schema_type("abc", ["integer", "string"]) == "abc"
    ...
    result = coerce_to_schema_type("invalid json", ["integer", "object"])
    assert result == "invalid json"  # Returns original value after all types fail

    # Test with three types
    assert coerce_to_schema_type("123.5", ["integer", "float", "string"]) == 123.5
    assert coerce_to_schema_type("true", ["integer", "boolean", "string"]) is True

    # Test with null in multi-type list
    assert coerce_to_schema_type("null", ["null", "string"]) is None
    assert coerce_to_schema_type("null", ["null", "object"]) is None

    # Test nested type conversion - boolean fails, integer succeeds
    value = coerce_to_schema_type("123", ["boolean", "integer", "string"])
    assert value == 123  # Should be integer, not boolean

    # coerce_to_schema_type uses fixed priority order (null > integer > ... > string),
    # so order in the list does not affect the result.
    assert coerce_to_schema_type("123", ["string", "integer"]) == 123
    assert coerce_to_schema_type("123", ["integer", "string"]) == 123

    # Test with all types failing - returns original value
    assert (
        coerce_to_schema_type("not_a_number", ["integer", "float", "boolean"])
        == "not_a_number"
    )
...
def test_convert_param_value_stricter_type_checking(parser):
    ...
    assert coerce_to_schema_type("yes", "boolean") == "yes"
    assert coerce_to_schema_type("no", "boolean") == "no"
    assert coerce_to_schema_type("TRUE", "boolean") is True
    assert coerce_to_schema_type("FALSE", "boolean") is False

    # Integer and float now raise exceptions for invalid values
    assert coerce_to_schema_type("123abc", "integer") == "123abc"
    assert coerce_to_schema_type("123.45.67", "float") == "123.45.67"

    # JSON parsing is stricter - invalid JSON returns original
    assert coerce_to_schema_type("{invalid: json}", "object") == "{invalid: json}"
    assert coerce_to_schema_type("[1, 2,", "array") == "[1, 2,"

    # Test multi-type with stricter checking
    # "yes" is not valid boolean, but string would accept it
    assert coerce_to_schema_type("yes", ["boolean", "string"]) == "yes"

    # "123abc" is not valid integer or float, but string accepts it
    assert (
        coerce_to_schema_type("123abc", ["integer", "float", "string"])
        == "123abc"
    )


def test_convert_param_value_edge_cases(parser):
    """Test edge cases for coerce_to_schema_type."""
    # Empty string
    assert coerce_to_schema_type("", "string") == ""
    assert (
        coerce_to_schema_type("", "integer") == ""
    ...
    assert coerce_to_schema_type('  { "key" : "value" }  ', "object") == {
        "key": "value"
    }
...
def test_convert_param_value_checked_helper(parser):
    """Test the coerce_to_schema_type helper function indirectly."""
    # This tests the behavior through the main function
    # Valid conversions should work
    assert coerce_to_schema_type("123", "integer") == 123
    assert coerce_to_schema_type("123.45", "float") == 123.45
    assert coerce_to_schema_type("true", "boolean") is True
    assert coerce_to_schema_type('{"x": 1}', "object") == {"x": 1}
    ...
    assert coerce_to_schema_type("null", "integer") is None
    assert coerce_to_schema_type("null", "boolean") is None
    assert coerce_to_schema_type("null", "object") is None
